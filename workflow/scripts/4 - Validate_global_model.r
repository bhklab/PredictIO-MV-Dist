...
data_files <- list.files(data_dir, pattern = "^testData.*\\.rda$", full.names = TRUE) # dat
score_files <- list.files(score_dir, pattern = "^testData.*_sig_score\\.rda$", full.names = TRUE) # dat$signatureData$signature
...
run_validation <- function(data_file, score_file, results_dir = "Validation/Results/") {
  ...
  for (model_file in model_files) {
    ...
    predMV_local <- predict(local_model, newdata = testExpr)
...
run_validation(data_file = "Validation/testDataHartwig_Bladder.rda",
                   score_file = "Validation/scores/testDataHartwig_Bladder_sig_score.rda",
                   results_dir = "Validation/")

# Run validation for all datasets
all_aucs <- list()
for (i in seq_along(data_files)) {
  result <- run_validation(data_file = data_files[i], score_file = score_files[i], results_dir = "Validation/Results/")
...
fig_dir <- "Validation/Additional_results/"
fig_files <- list.files(path = fig_dir, pattern = "^testDataHartwig_Bladder.*\\.jpeg$", full.names = TRUE)

# Loop through each image and save it as a single-page PDF
for (img_path in fig_files) {
  base_name <- tools::file_path_sans_ext(basename(img_path))
  
  pdf_path <- file.path("Validation/Results/PDF", paste0(base_name, ".pdf"))
  
  img <- image_read(img_path, density = "900x900")  # High-resolution read

  magick::image_write(img, path = pdf_path, format = "pdf", density = "900x900", quality = 100)
  
  message("Saved high-res PDF: ", pdf_path)
}
